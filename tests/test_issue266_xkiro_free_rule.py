...
from llm_discovery.pipeline import _is_free_model, _split_by_free_rule
...
class TestXkiroAccessTierRule:
    ...
    def test_is_free_model_free_tier(self):
        assert _is_free_model({"id": "m", "access_tier": "free"}, provider_name="xkiro") is True

    def test_is_free_model_paid_and_premium_not_free(self):
        assert _is_free_model({"id": "m", "access_tier": "paid"}, provider_name="xkiro") is False
        assert _is_free_model({"id": "m", "access_tier": "premium"}, provider_name="xkiro") is False
    ...
    def test_split_keeps_only_free_tier(self):
        ...
        keep, dropped = _split_by_free_rule(models, provider_name="xkiro")
...
class TestProviderScopedRegression:
    def test_non_xkiro_paid_premium_catalogue_kept(self):
        ...
        keep, dropped = _split_by_free_rule(models, provider_name="groq")
        assert keep == models
        assert dropped == []
